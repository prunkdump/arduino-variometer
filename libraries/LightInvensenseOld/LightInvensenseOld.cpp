...
int createCompressedFirmware(void) {
  ...
  VarioTW.readBytes(INV_HW_ADDR, INV_REG_LPF, 1, &data);
  Serial.print("#define COMPRESSED_DMP_LPF_CFG (0x");
  Serial.print(data, HEX);
  Serial.print(")\n");

  /* read sample rate (depend on DMP firmware) */
  VarioTW.readBytes(INV_HW_ADDR, INV_REG_RATE_DIV, 1, &data);
...
int fastMpuWriteMem(uint16_t memAddr, uint16_t length, const uint8_t *data) {
  ...
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_BANK_SEL, 2, com);

  /* write data */
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_MEM_R_W, length, data);
...
int fastMpuReadMem(uint16_t memAddr, uint16_t length, uint8_t *data) {
  ...
  /* write data */
  VarioTW.readBytes(INV_HW_ADDR, INV_REG_MEM_R_W, length, data);
...
void disableDMP(void) {
  ...
  /* clear */
  data = 0;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_INT_ENABLE, 1, &data);
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_FIFO_EN, 1, &data);
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_USER_CTRL, 1, &data);
...
void readMagSensAdj(void) {
  ...
  /* bypass */
  tmp[0] = BIT_BYPASS_EN;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_INT_PIN_CFG, 1, tmp); 

  /* get fuse access */
  tmp[0] = AKM_POWER_DOWN;
  VarioTW.writeBytes(LIGHT_INVENSENSE_COMPASS_ADDR, AKM_REG_CNTL, 1, tmp);
  delay(1);
  ...
  /* read values */
  VarioTW.readBytes(LIGHT_INVENSENSE_COMPASS_ADDR, AKM_REG_ASAX, 3, magSensAdj);
...
int fastMPUInit(bool startMPU) {
  ...
  while( resettingError && resetCountStep < MAX_INIT_RESET_RETRY ) {
    ...
    /* send reset command */
    data[0] = BIT_RESET;
    if( !VarioTW.writeBytes(INV_HW_ADDR, INV_REG_PWR_MGMT_1, 1, data) ) {
      resettingError = true;
    }      
    delay(100);
    ...
    /* read reset result */
    if( !resettingError ) {
      if( VarioTW.readBytes(INV_HW_ADDR, INV_REG_PWR_MGMT_1, 1, data) != 1 )
	resettingError = true;
    }
  ...
  /* Wake up chip. */
  data[0] = 0x00;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_PWR_MGMT_1, 1, data);
  ...
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_ACCEL_CFG2, 1, data);
#endif
  ...
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_GYRO_CFG, 1, data);

  /* set accel fsr */
  data[0] = LIGHT_INVENSENSE_ACCEL_FSR << 3;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_ACCEL_CFG, 1, data);

  /* set low pass filter */
  data[0] = COMPRESSED_DMP_LPF_CFG;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_LPF, 1, data);
  ...
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_PWR_MGMT_2, 1, data);
  delay(50);
  ...
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_PRGM_START_H, 2, data);
  ...
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_S0_ADDR, 1, data);
  data[0] = AKM_REG_ST1;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_S0_REG, 1, data);
  data[0] = BIT_SLAVE_EN | 8;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_S0_CTRL, 1, data);
  

  /* slave 1 : launch single measurement */
  /*
  data[0] = LIGHT_INVENSENSE_COMPASS_ADDR;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_S1_ADDR, 1, data);
  data[0] = AKM_REG_CNTL;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_S1_REG, 1, data);
  data[0] = BIT_SLAVE_EN | 1;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_S1_CTRL, 1, data);
  data[0] = AKM_SINGLE_MEASUREMENT;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_S1_DO, 1, data);
  */

  /* slave 4 :  launch single measurement */
  data[0] = LIGHT_INVENSENSE_COMPASS_ADDR;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_S4_ADDR, 1, data);
  data[0] = AKM_REG_CNTL;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_S4_REG, 1, data);
  //TODO check rate 
  data[0] = BIT_SLAVE_EN | ((1000/(1+COMPRESSED_DMP_RATE_DIV_CFG)) / LIGHT_INVENSENSE_COMPASS_SAMPLE_RATE - 1);
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_S4_CTRL, 1, data);
  data[0] = AKM_SINGLE_MEASUREMENT;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_S4_DO, 1, data);

  /* active slaves and set sample rate */
  //data[0] = 0x03;
  data[0] = 0x11;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_I2C_DELAY_CTRL, 1, data);
#ifdef MPU9150
  data[0] = BIT_I2C_MST_VDDIO;
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_YG_OFFS_TC, 1, data);
#endif //MPU9150
  /*
  ...
  */
  ...
  VarioTW.writeBytes(INV_HW_ADDR, INV_REG_RATE_DIV, 1, data);
...
int fastMPUReadFIFO(int16_t *gyro, int16_t *accel, int32_t *quat) {
  ...
  VarioTW.readBytes(INV_HW_ADDR, INV_REG_FIFO_COUNT_H, 2, data);
  ...
  if( fifoCount > (INV_HW_MAX_FIFO >> 1) ) {
    VarioTW.readBytes(INV_HW_ADDR, INV_REG_INT_STATUS, 1, data);
  ...
  /* read paquet */
  VarioTW.readBytes(INV_HW_ADDR, INV_REG_FIFO_R_W, COMPRESSED_DMP_PAQUET_LENGTH, dmpPaquet);
...
bool fastMPUMagReady(void) {
  ...
  uint8_t data;
  VarioTW.readBytes(INV_HW_ADDR, INV_REG_I2C_MST_STATUS, 1, &data);
...
int fastMPUReadRawMag(int16_t* mag) {
  ...
  /* read raw data */
  uint8_t tmp[9];
  VarioTW.readBytes(INV_HW_ADDR, INV_REG_RAW_COMPASS, 8, tmp);
