...
#define VARIOSCREEN_MUTE_WIDTH 8
const uint8_t varioscreenMute[] PROGMEM = {
  0x18,0x3c,0x7e,0x00,0x24,0x18,0x18,0x24 };
...
void ScreenMuteIndicator::display(void) {
  ...
  if( muted ) {
    displayElement(screen, varioscreenMute, posX, posY, VARIOSCREEN_MUTE_WIDTH, (VARIOSCREEN_FONT_HEIGHT/2));
  }

  /* else erase the symbol */
  else {
    for(int line = 0; line < (VARIOSCREEN_FONT_HEIGHT/2); line++) { 
      screen.beginDisplay(posX, posY + line);
